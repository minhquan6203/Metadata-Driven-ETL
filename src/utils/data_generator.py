...
class DataGenerator:
    ...
    def generate_transactions(
        ...
        for i in range(num_transactions):
            ...
            transaction_date = (
                start_datetime + datetime.timedelta(days=random_days)
            ).date()
    ...
    def generate_and_save_data(self, output_path: str = "/data/raw") -> None:
        ...
        customer_spark_df.write.mode("overwrite").csv(
            f"{output_path}/customers", header=True
        )
        ...
        end_date = datetime.datetime.now().strftime("%Y-%m-%d")
        start_date = (datetime.datetime.now() - datetime.timedelta(days=180)).strftime(
            "%Y-%m-%d"
        )

        # Get customer and product IDs
        customer_ids = [
            row.customer_id for row in customer_spark_df.select("customer_id").collect()
        ]
        product_ids = [
            row.product_id for row in product_spark_df.select("product_id").collect()
        ]
        ...
        transaction_spark_df.write.mode("overwrite").parquet(
