...
def initialize_metadata_tables(spark: SparkSession) -> None:
    ...
    empty_control_df.write.format("delta").mode("overwrite").option(
    ...
    empty_audit_df.write.format("delta").mode("overwrite").option(
        "overwriteSchema", "true"
...
def list_tables_by_layer(spark: SparkSession, layer: str) -> DataFrame:
    """
...
def register_table_schema(
    spark: SparkSession, table_name: str, schema: StructType, description: str = ""
) -> None:
    """
