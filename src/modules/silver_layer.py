...
class SilverLayer:
    ...
    def _process_transformation(
        self, transform: Dict[str, Any], run_date: str
    ) -> Dict[str, Any]:
        """
        ...
        df.write.format("delta").options(**write_options).mode("overwrite").saveAsTable(
            target_table
        )
    ...
    def _apply_data_quality(
        self, df: DataFrame, quality_rules: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        for rule in quality_rules:
            ...
            if action == "filter":
                ...
                if not validation_result["is_valid"]:
                    raise ValueError(
                        f"Data quality rule '{rule_name}' failed with reject action"
                    )
        ...
        return {"df": df, "metrics": metrics, "rejected_records": rejected_records}
