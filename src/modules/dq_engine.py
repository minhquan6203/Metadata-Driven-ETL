...
class DataQualityEngine:
    ...
    def validate_not_null(self, df: DataFrame, column: str) -> Dict[str, Any]:
        ...
        flagged_df = df.withColumn(
            f"__{column}_not_null_valid", F.col(column).isNotNull()
        )
    ...
    def validate_regex(
        self, df: DataFrame, column: str, pattern: str
    ) -> Dict[str, Any]:
        """
        ...
        # Create validation flag
        flagged_df = df.withColumn(
            f"__{column}_regex_valid", F.col(column).rlike(pattern)
        )
    ...
    def validate_uniqueness(self, df: DataFrame, columns: List[str]) -> Dict[str, Any]:
        ...
        flagged_df = (
            ...
            .join(
                df.groupBy(*columns)
                .count()
                .withColumnRenamed("count", "__count")
                .alias("b"),
                on=[F.col(f"a.{col}") == F.col(f"b.{col}") for col in columns],
                how="left",
            )
            .select(*[F.col(f"a.{col}") for col in df.columns], F.col("__count") == 1)
