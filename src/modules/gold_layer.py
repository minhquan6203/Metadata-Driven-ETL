...
class GoldLayer:
    ...
    def _process_model(self, model: Dict[str, Any], run_date: str) -> Dict[str, Any]:
        ...
        if write_mode == "merge" and "primary_keys" in model:
            ...
            df.write.format("delta").options(**write_options).mode(
                "overwrite"
    ...
    def _table_exists(self, table_name: str) -> bool:
        ...
        for table in tables:
            full_name = (
                f"{table.database}.{table.name}" if table.database else table.name
            )
    ...
    def _update_documentation(self, model: Dict[str, Any], target_table: str) -> None:
        ...
        if docs_records:
            ...
            docs_df.write.format("delta").mode("append").saveAsTable(
                documentation_table
            )
