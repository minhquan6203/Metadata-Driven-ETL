...
from pyspark.sql.types import StructType, StructField, StringType, TimestampType, IntegerType, DateType, BooleanType
...
def main():
    ...
    spark = SparkSession.builder \
        .appName("ETL Metadata Initialization") \
...
def create_data_quality_metrics_table(spark):
    ...
    empty_df.write \
        .format("delta") \
        .mode("overwrite") \
        .saveAsTable("metadata.data_quality_metrics")
...
def create_data_dictionary_table(spark):
    ...
    empty_df.write \
        .format("delta") \
        .mode("overwrite") \
        .saveAsTable("metadata.data_dictionary")
